...
d3.json('data6.json', function(err, data) {
	//console.log(data);
	...
	picto
		.selectAll('cirle')
		.data(data.entrykeysname)
		.enter()
		.append('circle')
		.attr('cx', function(d, i){ return pictoradiusbig + (i * width / data.entrykeysname.length) })
		.attr('cy', pictoheight / 2)
		.attr('r', pictoradiussmall)
		.attr('fill', d3.scaleOrdinal(d3.schemeCategory20))
		.on('mouseover', function(d){
			d3
				.select(this)
				.transition()
				.ease(d3.easeQuadInOut)
				.attr('r', pictoradiusbig);
			d3
				.select('#subtitle')
				.text(d.toUpperCase());
		})
		...
		.on('click', function(d){
			...
			selectedsimilardocid = entrykeys[d];
