...
contextkeys.strokeStyle = 'rgb(20, 30, 40)';
...
d3.json('data7.json', function(err, data) {
	//console.log(data);
...
function drawkeys(){
	contextkeys.clearRect(0, 0 , width, 2 * canvasheight);
	Object.keys(entrykeys)
		.map(function(d, i){
			var a = fisheye(i * (2 * canvasheight / Object.keys(entrykeys).length), distorsionkeys, keysmouseY, 2 * canvasheight);
			var b = fisheye((i + 1) * (2 * canvasheight / Object.keys(entrykeys).length), distorsionkeys, keysmouseY, 2 * canvasheight);
			
			contextkeys.font = (keysmouseY > a & keysmouseY < b) ? ((b - a) / 3) + 'px roboto black, verdana' : ((b - a) / 3) + 'px roboto, verdana';
			contextkeys.fillStyle = (clickedlabels.includes(d)) ? 'rgba(170, 200, 110, 1)' : (keysmouseY > a & keysmouseY < b) ? 'rgba(180, 40, 60, 1)' : 'rgba(256, 256, 256, 1)';
...
canvashiddenkeys
	...
	.on('click', function(){
		...
		hiddencontextkeys.clearRect(0, 0, sidedivwidth, 2 * canvasheight);
		//
