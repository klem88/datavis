...
var urldescription = 'http://univ.scholarvox.com/sapi/info?docid=';
...
function distorsionin(arraylength){ return d3.interpolateNumber(0, arraylength * distorsionfactor * widthscale(width)) };
function distorsionout(arraylength) { return d3.interpolateNumber(arraylength * distorsionfactor * widthscale(width), 0) };
...
var keyscanvasheight1 = (canvasheight * 2) * 2 / 3;
var keyscanvasheight2 = (canvasheight * 2) * 1 / 3;
...
d3
	.select('#inputfields')
	.style('top', keyscanvasheight1 + 'px')
	.style('width', sidedivwidth + 'px');
...
d3.select('#submit').on('click', function(){
	...
	if (username != '' & pwd != ''){
		BookshelfMgr.login(username, pwd, function(){
			...
			d3.select('#canvashiddenkeys2').style('visibility', 'visible');
			BookshelfMgr.folders(userfolders);
		});
	} else {alert('Username and Password are needed')};
});

function userfolders(userfolders){
	userfolders.map(function(d, i){
		var col = generatecolor();
		datacolorassoc[col] = d.name;
		datacolorassoc[d.name] = col;

		BookshelfMgr.folder_books(d.id, function(userfolderbooks){
			var arraydocid = [];
			userfolderbooks.items.map(function(e){
				arraydocid.push(e.docid);
			})
			userentrykeys[d.name] = arraydocid;
			(i == userfolders.length - 1) ? drawkeys() : null;
		});
	});
};
...
// LOG COVER POSITION SCALES
var xscale = d3
	.scalePow() // exponent depends on the number of covers in the accordion
	.range([0, width])
	.domain([0, width]);

// DISTORSION SCALE
var distorsionscale = d3
	.scalePow() // exponent and range depends on the number of covers in the accordion
	.domain([0, width]);

// EXPONENT SCALE
var exponentscale = d3
	.scaleLinear()
	.range([1, 2])
	.domain([1, 100]) // Un accordéon contient entre 1 et 100 couvertures
...
d3.json(datafile, function(err, data) {
	...
	data.docid.map(function(d, i){
		...
		//distmat[d] = data.simdocid[i].filter(function(d){ return d !== null });
	});
...
function drawkeys(){
	...
	Object.keys(entrykeys)
		.map(function(d, i){
			...
			contextkeys1.font = (keysmouseY1 > a & keysmouseY1 < b) ? ((b - a) / 30) + 'vw roboto black, verdana' : ((b - a) / 30) + 'vw roboto, verdana';
	...
	Object.keys(userentrykeys)
		.map(function(d, i){
			var a = fisheye(i * (keyscanvasheight2 / Object.keys(userentrykeys).length), distorsionkeys2, keysmouseY2, keyscanvasheight2);
			var b = fisheye((i + 1) * (keyscanvasheight2 / Object.keys(userentrykeys).length), distorsionkeys2, keysmouseY2, keyscanvasheight2);
			
			//contextkeys2.font = (keysmouseY2 > a & keysmouseY2 < b) ? ((b - a) / 2) + 'px roboto black, verdana' : ((b - a) / 2) + 'px roboto, verdana';
			contextkeys2.font = (keysmouseY2 > a & keysmouseY2 < b) ? ((b - a) / 30) + 'vw roboto black, verdana' : ((b - a) / 30) + 'vw roboto, verdana';
			var width = contextkeys2.measureText(d.toUpperCase()).width + 10;

			contextkeys2.fillStyle = 'rgba(20, 30, 40, 0.7)'//'rgba(256, 256, 256, 0.8)';
			contextkeys2.fillRect(0, a, width, b - a);
	
			contextkeys2.fillStyle = (clickedlabels.includes(d)) ? 'rgba(20, 120, 10, 1)' : (keysmouseY2 > a & keysmouseY2 < b) ? 'rgba(245, 65, 55, 1)' : 'rgba(256, 256, 256, 1)';
...
function draw(){
	...
	var adjust = exponentscale(goodcoverslength) * widthscale(width) * heightscale(height);

	xscale.exponent(1 / adjust);
	distorsionscale
		.exponent(adjust)
		.range([0.2 / adjust, adjust * 1.5]); // Facteur manuel -- correspond à la distorsion augmentée au fur et à mesure de l'accordéon


	selectedsimilardocid
		.filter(function(d){ return !erroredcovers.includes(d); })
		.map(function(d, i){
			if(goodcoverslength > 5){
				var a = fisheye(xscale(i / goodcoverslength * width), distorsion * distorsionscale(mouseX), mouseX, width);
				var b = fisheye(xscale((i + 1) / goodcoverslength * width), distorsion * distorsionscale(mouseX), mouseX, width);
			} else {
			...
			cover.onload = function(){	
				...
				context.beginPath();
				context.rect(a, 0, b - a, picheight);
				context.lineWidth = Math.max(2, ((b - a) - picwidth));
				context.stroke();
				context.clip();
...
function drawhidden(){
	selectedsimilardocid
		...
		.map(function(d, i){
			if(goodcoverslength > 5){
				var a = fisheye(xscale(i * width / goodcoverslength), distorsion * distorsionscale(mouseX), mouseX, width);
				var b = fisheye(xscale((i + 1) * width / goodcoverslength), distorsion * distorsionscale(mouseX), mouseX, width);
			} else {
				var a = fisheye(i * width / goodcoverslength, distorsion, mouseX, width);
				var b = fisheye((i + 1) * width / goodcoverslength, distorsion, mouseX, width);
			};
...
canvashiddenkeys2
	.on('touchstart mouseenter', function(){
		...
		canvaskeys2
			...
			.tween('', function(){
				return function(t){
					distorsionkeys2 = distorsionin(Object.keys(userentrykeys).length)(t);
					drawkeys();
				}
			});
	})
	...
	.on('touchend mouseleave', function(){
		...
		canvaskeys2
			...
			.tween('', function(){
				return function(t){
					distorsionkeys2 = distorsionout(Object.keys(userentrykeys).length)(t);
					drawkeys();
	...
	.on('click', function(){
		...
		// INSERT THE SELECTED KEY DOCIDS INTO THE ARRAY OF SELECTED SIMILAR DOCIDS
		selectedsimilardocid = userentrykeys[datacolorassoc[colString]];
...
canvasselectedcovers
	...
	.on('click', function(){
		if(selectedcovers.length > 0){
			...
			// NEW ARRAY OF DOCID
			getsimdocid(currentbookselected); //selectedsimilardocid = distmat[currentbookselected];
...
/*function logfisheye(x, disto, mouseXcoord, max){
	var left = x < mouseXcoord;
...
function getsimdocid(docid){
	$.getJSON("http://datavis.it/api/voxuniv/getsimdocid?docid=" + docid, function(res){
    	selectedsimilardocid = res.simdocid;
		drawprep();
	});
}